...
const themeOptions: { value: ThemeMode; label: string; icon: React.FC }[] = [
...
const ThemeMenu : React.FC = () => {  
  ...
  const current = themeOptions.find(opt => opt.value === mode) || themeOptions[2]
  return (
                  ...
                  {({ active }) => (
                    <button
                      className={`w-full flex items-center gap-2 px-4 py-2 text-left text-base rounded-lg transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-indigo-400
                        ...
                        ${mode === opt.value ? 'font-bold text-indigo-700 dark:text-indigo-300 bg-indigo-100 dark:bg-gray-800' : 'text-gray-700 dark:text-gray-300'}`}
