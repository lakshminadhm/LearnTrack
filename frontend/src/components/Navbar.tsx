...
import { Menu, X, BookOpen, BarChart2, ClipboardList, Target, Users, LogOut, LogIn, UserPlus, GraduationCap, Settings, Sun, Moon, Monitor, ChevronDown } from 'lucide-react';
    ...
    <nav className="sticky top-0 z-40 bg-white/80 dark:bg-gray-900/80 backdrop-blur-md shadow-lg transition-all border-b border-gray-200 dark:border-gray-800">
      <div className="w-full max-w-screen-xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          {/* Desktop expanded menu - hidden on smaller screens */}
          ...
          {/* Desktop collapsed menu button (md screens) */}
          <div className="hidden md:flex lg:hidden items-center">
            <div className="relative" ref={desktopMenuRef}>
              <button 
                type="button" 
                className="inline-flex items-center justify-center p-2 rounded-md text-gray-500 hover:text-indigo-700 hover:bg-indigo-50 dark:text-gray-400 dark:hover:text-indigo-300 dark:hover:bg-gray-800/70 focus:outline-none focus:ring-2 focus:ring-inset focus:ring-indigo-500" 
                aria-expanded={isDesktopMenuOpen} 
                onClick={toggleDesktopMenu}
              >
                <span className="sr-only">{isDesktopMenuOpen ? 'Close menu' : 'Open menu'}</span>
                <div className="relative w-6 h-6">
                  {/* Hamburger to X animation */}
                  <span 
                    className={`absolute block h-0.5 w-6 bg-current transform transition duration-300 ease-in-out ${
                      isDesktopMenuOpen ? 'rotate-45 translate-y-0' : '-translate-y-2'
                    }`}
                  ></span>
                  <span 
                    className={`absolute block h-0.5 w-6 bg-current transform transition duration-300 ease-in-out ${
              ...
              {isDesktopMenuOpen && (
                <div className="absolute right-0 mt-2 py-2 w-56 bg-white dark:bg-gray-900 rounded-lg shadow-xl border border-gray-200 dark:border-gray-800 z-50">
                  {navLinks.map((link) => (
                    (!link.auth || isAuthenticated) && (!link.adminOnly || isAdmin) && (
                      <Link
                        key={link.path}
                        to={link.path}
                        className={`flex items-center gap-2 px-4 py-2 text-sm font-medium transition-all duration-200 hover:bg-indigo-50 dark:hover:bg-gray-800/70 ${location.pathname === link.path ? 'bg-indigo-100 text-indigo-700 dark:bg-gray-800 dark:text-indigo-300' : 'text-gray-700 dark:text-gray-300'}`}
                    ...
                    link.show && (
                      link.action ? (
                        <button
                          key={`auth-${i}`}
                          onClick={link.action}
                          className="flex w-full items-center gap-2 px-4 py-2 text-sm font-medium text-gray-700 dark:text-gray-300 hover:bg-indigo-50 dark:hover:bg-gray-800/70 transition-all duration-200"
                        >
                          {link.icon}
                          <span>{link.label}</span>
                        </button>
